// cargo run --example poisson_quad
...
use mesh_sieve::mesh_generation::{MeshGenerationOptions, quad_mesh};
...
fn main() -> Result<(), MeshSieveError> {
    ...
    discretization.field_mut("u").set_cell_type_metadata(
        ...
        DiscretizationMetadata::new("lagrange_q1", "gauss2"),
    );
    let field = discretization
        .field("u")
        .ok_or_else(|| MeshSieveError::InvalidGeometry("missing field metadata".to_string()))?;
    let metadata = field
        .metadata_for(&RegionKey::cell_type(CellType::Quadrilateral))
        .ok_or_else(|| MeshSieveError::InvalidGeometry("missing quad metadata".to_string()))?;
    let runtime = runtime_from_metadata(metadata, CellType::Quadrilateral)?;
    ...
    for (cell, slice) in cell_types.iter() {
        if slice.first() != Some(&CellType::Quadrilateral) {
            continue;
        }
    ...
    let num_dofs = dof_map.len();
    for i in 0..num_dofs {
        for j in 0..num_dofs {
            let diff = (matrix_rows[i][j] - matrix_rows[j][i]).abs();
            assert!(diff < 1.0e-12, "matrix asymmetry at ({i},{j})");
        }
    }
    ...
    assert!(rhs_values.iter().all(|v| *v > 0.0), "rhs positivity check");
