    ...
    G: PartitionableGraph<VertexId = usize> + Sync,
{
    ...
    let mut vertices: Vec<usize> = graph.vertices();
...
mod tests {
    ...
    impl PartitionableGraph for PathGraph {
        ...
        fn neighbors(&self, v: usize) -> Vec<Self::VertexId> {
            ...
            if v > 0 { neigh.push(v - 1); }
            if v + 1 < self.n { neigh.push(v + 1); }
            neigh
        }
    ...
    fn pick_seeds_path_highest_degree() {
        ...
        let cfg = PartitionerConfig { n_parts: 1, seed_factor: 1.0, ..Default::default() };
        let seeds = pick_seeds(&g, &degrees, &cfg);
        // For a path, the highest degree is in the middle. This is probabilistic, so just check that the seed count is correct and all are valid.
    ...
    fn pick_seeds_equal_degrees() {
        ...
        let cfg = PartitionerConfig { n_parts: 2, seed_factor: 1.0, ..Default::default() };
        let seeds = pick_seeds(&g, &degrees, &cfg);
    ...
    fn pick_seeds_more_than_n() {
        ...
        let cfg = PartitionerConfig { n_parts: 5, seed_factor: 1.0, ..Default::default() };
        let seeds = pick_seeds(&g, &degrees, &cfg);
