...
where
    ...
    N: Neigh<'a, P>,
{
    let Opts { dir, strat, max_depth, deterministic, early_stop } = opts;
    ...
    match strat {
        Strategy::DFS => {
            ...
            while let Some((p, d)) = stack.pop() {
                ...
                let push = |it: N::Iter, dnext: u32, seen: &mut HashSet<P>, stack: &mut Vec<(P,u32)>| {
                    for q in it {
                        if seen.insert(q) { stack.push((q, dnext)); }
                    }
                };
                match dir {
        ...
        Strategy::BFS => {
            ...
            while let Some((p,d)) = q.pop_front() {
                if let Some(f) = early_stop { if f(p) { break; } }
                if max_depth.map_or(false, |md| d >= md) { continue; }
                let push = |it: N::Iter, dnext: u32, seen: &mut HashSet<P>, q: &mut VecDeque<(P,u32)>| {
                    for qn in it {
                        if seen.insert(qn) { q.push_back((qn, dnext)); }
                    }
                };
                match dir {
