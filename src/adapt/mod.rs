...
pub struct MetricNormalizationControls {
    ...
    /// Maximum allowed cell-to-cell metric gradation ratio.
    pub gradation: Option<f64>,
    /// Hausdorff tolerance carried for remeshing backends.
    pub hausdorff_number: Option<f64>,
    /// Lower bound for edge anisotropy ratios.
    pub min_anisotropy: Option<f64>,
    /// Upper bound for edge anisotropy ratios.
    pub max_anisotropy: Option<f64>,
    /// Lower bound for tensor diagonal magnitudes.
...
/// External remeshing engines anticipated by the metric-adaptation facade.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExternalRemeshingBackend {
    /// Triangle-based 2D constrained Delaunay remeshing.
    Triangle,
    /// TetGen-based 3D tetrahedral remeshing.
    TetGen,
    /// Gmsh metric/remeshing pipeline.
    Gmsh,
    /// MMG anisotropic remeshing pipeline.
    Mmg,
}
...
pub enum MetricRemeshingBackend {
    /// Use mesh-sieve's in-process refinement/coarsening primitives.
    Internal,
    /// Request a concrete external remesher. Returns a clear error until linked.
...
/// Explicit old/new point provenance generated by adaptation.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct AdaptationProvenanceMap {
    /// For each input point, the output points that inherit its data.
    pub old_to_new: Vec<(PointId, Vec<PointId>)>,
    /// For each output point, the input points used as its data source.
...
pub struct MetricAdaptationOptions {
    ...
    /// DMPLEX-style metric normalization controls applied before decisions.
    pub normalization: MetricNormalizationControls,
    /// Remeshing backend policy.
...
/// Normalize a per-cell metric section with DMPLEX-style controls.
...
pub struct MetricAdaptationResult<V> {
    ...
    /// Explicit point provenance for transferring metadata through the pass.
