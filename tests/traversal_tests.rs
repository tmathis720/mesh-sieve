...
fn p(x: u64) -> PointId { PointId::new(x + 1).unwrap() }
...
fn parity_closure_star() {
    ...
    let cl = TraversalBuilder::new(&s).dir(Dir::Down).dfs().seeds(seeds).run();
    assert_eq!(cl_ref, cl);
    let star_ref_v = star_ref(&s, [p(2)]);
    let star_v = TraversalBuilder::new(&s).dir(Dir::Up).dfs().seeds([p(2)]).run();
...
fn dir_both_union() {
    let s = build_line();
    let seeds = [p(1)];
    let down = TraversalBuilder::new(&s).dir(Dir::Down).dfs().seeds(seeds).run();
    let up = TraversalBuilder::new(&s).dir(Dir::Up).dfs().seeds(seeds).run();
    let mut union = down.clone();
    union.extend(up.iter().copied());
    union.sort_unstable();
    union.dedup();
    let both = TraversalBuilder::new(&s).dir(Dir::Both).dfs().seeds(seeds).run();
...
fn determinism_shuffle() {
    ...
    let a = TraversalBuilder::new(&s).dir(Dir::Down).dfs().seeds(seeds.clone()).run();
    seeds.reverse();
    let b = TraversalBuilder::new(&s).dir(Dir::Down).dfs().seeds(seeds.clone()).run();
    assert_eq!(a, b);
